...
def sample_coordinates_max(ranges, y_max, x_max, seed=None):
    ...
    while True:
        cube_position = rng.uniform(ranges[:, 0], ranges[:, 1])
        if not (cube_position[0] > x_max  or  cube_position[1] > y_max):
            break
...
def sample_put_in_box_pose(seed=None):
    ...
    z_range = [0.05, 0.05]
    ranges = np.vstack([x_range, y_range, z_range])
    cube_position = sample_coordinates(ranges, -0.07, -0.07, seed)
...
def sample_block_stacking(seed=None):
    ...
    to_return=[]
    cube_quat = np.array([1, 0, 0, 0])
    base_position = sample_coordinates_mid(ranges, -0.06, 0.06, -0.06,0.06 , seed)
    to_return.append(np.concatenate([base_position, cube_quat]))
    beam_position = sample_coordinates_mid(ranges, -0.1, 0.03, -0.1, 0.1, seed)
    to_return.append(np.concatenate([beam_position, cube_quat]))
    block1_position = sample_coordinates_mid(ranges, 0.05, 0.1, -0.1, -0.01, seed)
    to_return.append(np.concatenate([block1_position, cube_quat]))
    block2_position= sample_coordinates_mid(ranges, 0.05, 0.1, 0, 0.1, seed)
